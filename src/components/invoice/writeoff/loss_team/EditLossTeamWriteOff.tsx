...
export default function EditLossTeamWriteOff({
  setShowEditModal,
  invoiceWriteOff,
}: Props) {
  ...
  useEffect(() => {
    if (allTeamsQuery.isSuccess && allTeamsQuery.data) {
      ...
      const team = allTeamsQuery.data.find(val => val.id == invoiceWriteOff.writeOffLocationID)
      if (team) {
        setWriteOffLocation({
  ...
  const invoiceMaterialsForEditQuery = useQuery<IInvoiceWriteOffMaterials[], Error, IInvoiceWriteOffMaterials[]>({
    ...
    queryFn: () => getInvoiceWriteOffMaterialsForEdit(invoiceWriteOff.id, "team", writeOffLocation.value),
  ...
  const materialQuery = useQuery<InvoiceWriteOffMaterialsForSelect[], Error, InvoiceWriteOffMaterialsForSelect[]>({
    ...
    queryFn: () => getUniqueMaterialsInLocation("team", writeOffLocation.value),
    enabled: writeOffLocation.value != 0,
  })
  ...
  const updateInvoiceWriteOffMutation = useMutation<InvoiceWriteOffMutation, Error, InvoiceWriteOffMutation>({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["invoice-writeoff", "loss-team"])
          ...
          <div className="flex space-x-2 items-center w-full">
            ...
            {allTeamsQuery.isSuccess &&
              <div className="flex flex-col space-y-1">
                <label htmlFor="warehouse-manager">Бригада</label>
